...
2. **Loading a large graph over the network** with the ratified network bulk-import (Mode A) —
   `CREATE DATABASE` → streaming CSV upload → `START DATABASE` → `CREATE INDEX` — rather than the
   `O(E·N)` per-edge Cypher path that does not scale.
3. **Read scaling under concurrency**: a ladder of increasing simultaneous connections, each running
...
4. **A read-heavy / write-light mix**: a small, rate-limited stream of new purchases runs
...
| `(:Product {id, name, category, price})` | `id` (24-hex, unique) | a catalogue item (`price` in cents) |
...
profile" recommendation needs to be meaningful. The generator (`reco_gen`) is fully deterministic:
identical seed ⇒ byte-identical CSV, across runs and platforms (integer-only math, no floats on the
wire).

## The recommendation read battery

Every query anchors on `(:User {id: $id})`, an index seek, with `$id` varying per operation
(see [`src/queries.rs`](../../crates/graphus-reco-gen/src/queries.rs)):
...
bulk-import upload), loads and indexes the graph, asserts the shape and that every recommendation
query is well-formed, drives the concurrency ladder, writes the evidence, gates the fast profile
...
- **`reco_load`** — the REST-only loader: `POST /auth/login` → `CREATE DATABASE` → streaming
  bulk-import (Mode A) → `START DATABASE` → `CREATE INDEX` → shape + recommendation-query asserts.
...
schema shared by every example) + `report.md` (human-readable):
