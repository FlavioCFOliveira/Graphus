...
## Post-ratification note (2026-08-13) — `D-version-representation`

> **This note records how task #1069 completed the mechanism `D-version-representation` was ratified
> on. It creates no new decision, and it changes no ratified outcome of the 2026-08-02 round.** The
> newest version stays in the home record, older versions are still reconstructed by walking one
> unified undo-delta chain per entity, every mutation kind is still a delta on that one chain, and the
> chain is still anchored by the record's `undo_ptr`.
>
> **What changed.** The **record header** now resolves through the commit indirection point, exactly
> as a delta already did. An unsettled `created_ts` / `expired_ts` no longer carries the writing
> transaction's `TxnId`; it carries the physical id of that transaction's slot in `commit.store`. The
> bit layout is untouched — same `0` sentinel, same high-bit discriminant, same 63-bit payload — so
> the change is one of **meaning**, and the on-disk format version moved from 5 to **6** to make the
> two conventions distinguishable.
>
> **Why it is this decision that the change belongs to.** `D-version-representation` was ratified with
> the commit indirection point as the load-bearing part of its design: one slot per transaction, one
> atomic store at commit, every delta of that transaction committed at the same instant. The record
> header was the **one axis of the store that the indirection did not cover**, and everything that
> existed to keep it resolvable — the per-record freeze sweep, the `freeze_low` frontier, and the WAL
> reclamation floor that holds a committed transaction's log record down until its stamps are settled
> — existed to serve a *second* commit oracle, in memory. The store now has one.
>
> **Where this lands against the references the decision cited.** The ratified row names "the Memgraph
> / InnoDB model" for the version representation as a whole. On the narrower question of *what a
> record names*, that pairing now splits, and Graphus has moved to one side of it:
>
> | System | What the record carries | What resolves it |
> | --- | --- | --- |
> | **InnoDB** (`/data/refsrc/mysql-server`, tag `mysql-8.0.36`) | the clustered-index record carries `DATA_TRX_ID` (6 bytes, the transaction id) beside `DATA_ROLL_PTR` (7 bytes, the undo pointer) — `storage/innobase/include/data0type.h:181-190` | `ReadView::changes_visible(trx_id_t, …)`, `storage/innobase/include/read0types.h:162-182`, against an **in-memory** read view holding `m_up_limit_id`, `m_low_limit_id`, `m_creator_trx_id` and a sorted vector of active ids |
> | **Memgraph** (`/data/refsrc/memgraph`) | the record carries **no version stamp at all**: `Vertex` holds a `gid`, a deleted bit and a delta pointer (`src/storage/v2/vertex.hpp:33-65`) | the delta's shared `CommitInfo` (`src/storage/v2/delta.hpp:40-49`), loaded on every read (`src/storage/v2/mvcc.hpp:57`) |
> | **Graphus, since #1069** | a fixed-size durable record header, whose unsettled stamp names the writer's **commit slot** | that slot, read from `commit.store` |
>
> Graphus is off the InnoDB shape — "the record names a transaction, resolved against an in-memory
> table" — and onto Memgraph's indirection, with one deliberate divergence: Memgraph's delta holds an
> in-memory **pointer** to a `CommitInfo` and its record holds nothing, while Graphus's header holds a
> **durable slot id**. That divergence is the point. It is what makes a captured image resolvable
> without the process that wrote it. (The 2026-08-02 evidence table records that there was no InnoDB
> source tree in `/data/refsrc` at the time and that nothing about InnoDB was read from code; the tree
> is present now, and the InnoDB row above **was** read from it.)
>
> **The consequence that is a correctness fix, not a refactor.** A restored store opens over a fresh,
> empty WAL. Before #1069, any version whose stamp had not been settled before capture named a writer
> that the restored store's in-memory table could not translate — and an unknown writer resolves as
> aborted, so a **committed version read as invisible**. A full backup hid this by freezing every
> header before capture (task #149); a restored **chain** could not, because its increments carry
> pages that no freeze of the base ever saw. Committed data was silently absent after a chain restore.
> The image now carries its own oracle, `commit.store` is one of the stores it captures, and the
> pre-capture freeze is retired.
>
> **Three things this note does not claim.** The freeze sweep, the frontier and the WAL reclamation
> floor are **not removed** — #1069 demoted them from correctness to performance, and task **#1070**
> removes them. `CommitSlot.commit_ts` keeps a `TxnId` payload and is **not** part of the change; it
> shares the bit layout with the header and must never be resolved by the header's oracle. And the
...
> replaced by `graphus_txn::is_visible_via`, which takes a commit oracle and returns a `Result`
> because resolving a stamp now reads a page and may fail. Every such passage keeps its meaning
> exactly; only the name and the fallibility are new, and no ratified outcome depends on either.
>
> Design in `04-technical-design.md` §5.1.3, §5.2, §5.3 and §5.6; on-disk form in
> `05-storage-format.md` §7, §11, §12.4 and §12.6.

## Post-ratification note (2026-08-13) — `D-orphan-slot-parking`

> **This note records that task #1069 brought a second kind of slot under the parking discipline. It
> creates no new decision and changes no ratified outcome of the 2026-08-05 round.** A record slot
> that a logical rollback orphans is still parked in memory until the next garbage-collection pass,
> still for the reason ratified: the latest-state TEXT, FULLTEXT and SPATIAL indexes are in memory,
> not transactional, and keyed by physical node id.
>
> **What was added.** A **commit-info slot** (`05-storage-format.md` §12.4) is now parked by the same
> mechanism: the garbage collector's reference census proves a slot unreachable and parks its id, and
> the id is listed on the free list at the start of a **later** pass, never in the instant it stopped
> being named.
>
> **The reason is different, and sharper.** For a record slot, immediate recycling resurrects a stale
> index posting — a false positive that a re-check filters, until it turns an insert into what the
> index reads as the replacement of a still-committed document. For a commit slot, immediate recycling
> **re-attributes a committed version to a different transaction**: a record header that still names
> the id would resolve through whatever transaction took the slot next. The failure is a wrong answer
> about committed data rather than a degraded index.
>
> **One asymmetry worth stating.** For a record slot, parking is a restraint on the abort path, which
> could have freed the id itself. For a commit slot, parking is the **only** route to the free list:
> since #1069 no other code path returns a `commit.store` id to circulation, and every path that
> retires a slot merely clears its in-use bit and arms the census.
>
