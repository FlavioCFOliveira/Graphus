...
| Two writers **lowering the freeze frontier** at the same instant, and a freeze sweep or a rollback's savepoint restore landing on top of one (`#1014`). Unreachable in DST for the same reason as the row above, and unreachable *behaviourally* even with threads on x86: TSO makes the window between the load and the store of a naive descent so narrow that three separately designed racing tests never observed the lost update the mechanism is there to prevent | `graphus-freezefloor/tests/loom_freezefloor.rs` (three positive models — concurrent descents, a raise over a descent, a rollback restore over a descent — each paired with a **negative control** that performs the same scenario with the naive operation and is required to lose a descent on at least one explored interleaving) |
