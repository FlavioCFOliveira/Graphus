...
| Thread-scheduling seam | `graphus-core` (`sched`) | The yield points at which a **real OS thread** offers the execution token back: page-latch acquisition and release, thread spawn/exit, a commit's slot publication and horizon advance, snapshot acquisition, free-list push, and each GC phase. Behind the `det-sched` cargo feature, off by default — see section 5.2. |
...
Until task #1070 this table also carried the **freeze frontier** of task #1014 — two writers lowering it
at the same instant — owned by the `loom` models of the `graphus-freezefloor` crate. Task #1070 removed
the frontier, the crate and its models, so the row went with the mechanism it described.
...
| Commit publication | The **durable** slot publication (`CommitPublishSlot`), the advance of the commit-visibility horizon over the commit's timestamp (`CommitPublishVisible`), and the settling of the active-set entry (`CommitSettle`) — three sites, not one, because the windows *between* them are exactly what a scheduled reader must be able to observe. Since task **#1069** a record header becomes resolvable at the **first** of the three, so what the other two leave open is no longer a header's visibility: it is the window the published snapshot horizon of `04-technical-design.md` §5.2 closes. The second site was named `CommitRegistryRecord` until task **#1071** removed the in-memory commit registry written at that instant; its code, `41`, is unchanged, so a recorded schedule replays identically |
| Snapshot acquisition | The reclamation floor every GC watermark derives from, the snapshot an off-thread read task carries away, and — since task **#1056** — the **begin snapshot** a transaction takes as its start timestamp. The last one is not symmetry: a transaction that takes its snapshot while another worker sits between issuing a commit timestamp and publishing its commit is the window the published horizon of `04-technical-design.md` §5.2 closes, and without a yield point at the snapshot itself no seed can place a transaction there — `begin` runs straight through from wherever its thread was last parked, so the first by-seed reproduction of that defect came back clean on every seed with the horizon withdrawn |
| Storage | Free-list push (a slot becomes reusable), undo-chain-head publication |
| Garbage collection | Phases A, B, C, B2, F, D and E **individually** — not the pass as a whole. Since task **#1070** the phase-E site sits inside the settle-and-census walk, once per MVCC record store, because that walk replaced the freeze sweep phase E used to name. The pass is the coarsest possible grain and would make every reclamation atomic with respect to a reader, which is exactly the interleaving the `rmp` #811 class lives inside |
...
| `graphus-dst/tests/det_scheduler_census_window_1070.rs` | Over a writer that commits continuously (and aborts every seventh transaction) while full GC passes run, no committed row is lost; over a writer that overwrites cells in place while settle-only passes run, no settle overwrites a writer's stamp. Each sweep asserts that a writer commit landed inside a GC pass, so the window was entered. **What it catches, and what it does not.** It catches the commit-slot census race as it shipped before the census window existed (committed rows lost on 40 of 40 seeds) and the settle race (an unconditional settle fails 6 of 40 seeds). It does **not** catch the removal of any single one of the four census-window facts of `05-storage-format.md` §12.4 from the fixed store: the yield points almost never place a writer's allocation, registration and first reference on the precise sides of the window that one fact alone guards. Each fact is pinned instead by a single-threaded unit test in `graphus-storage`'s `store.rs` that places the writer in that window by hand and is red when only its own fact is removed |
