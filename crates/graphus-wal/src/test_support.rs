...
/// `read_bounded` produced (`into.len()`), forwarding every operation to the wrapped sink otherwise.
///
/// It lets a test prove the recovery reads stay bounded by the RETAINED window and never allocate a
/// buffer sized by the log's whole lifetime (`rmp` #525): reverting the recovery-path
...
impl<S: LogSink> CountingSink<S> {
    /// Wraps `inner`, publishing its largest observed read into the shared `max_read` counter.
    pub(crate) fn new(inner: S, max_read: Arc<AtomicU64>) -> Self {
        Self { inner, max_read }
    }
...
impl<S: LogSink + Clone> Clone for CountingSink<S> {
    fn clone(&self) -> Self {
        // Clones share the SAME counter (an `Arc`), so a `WalManager::open` on a cloned sink still
        // publishes its reads to the test's observation point.
        Self {
