...
//!
//! It loads a committed **baseline** evidence report and a **fresh** `social_bench` `report.json`, then
...
//! metrics and a clear diff and exits `1`. The social-network `run.sh` invokes it as the
//! committed-baseline gate against a `social_bench` `report.json` (LOCAL fast profile only).
//!
//! ## Why a STRUCTURAL-only comparison
//!
//! Everything `social_bench` reports about *performance* — throughput, latency percentiles, server CPU
//! seconds / core utilisation, peak RSS, disk IO, the writer abort rate — is **machine- and
//! timing-dependent**: it varies with the host's core count, its allocator, kernel scheduling, and the
//! load the box is under, so comparing those numbers across machines is inherently flaky. This gate
//! therefore holds ONLY the metrics that are **deterministic for a fixed seed + generator profile** —
//! the realised node and relationship counts and the four per-label structural counts
//! (`USER`/`ARTICLE`/`FRIEND`/`LIKE`) the driver was told the graph contains — and gives every volatile
//! metric family an effectively-infinite tolerance.
//!
//! Unlike the old in-process gate, there is **no** durable-store byte gate here: `social_bench` is a
//! **client-only** driver (it never links the engine, so it measures no on-disk store image — the
//! server writes it out of process). The structural counts are the only deterministic signal, and they
//! must match the baseline **exactly**.
//!
//! Hermetic: this binary needs only the harness + serde (NO engine feature), so it is always buildable
//! — `cargo build -p graphus-social-gen --no-default-features --bin social_baseline_cmp` builds it.
...
fn main() -> ExitCode {
    ...
    // run to surface an (impossible) regression clearly should the thresholds ever be tightened.
    ...
    // HONESTY: the perf/resource families above are UNGATED here (threshold = ∞): an absolute baseline
    // is host-specific and this suite runs on wildly different hardware. That is deliberate, NOT a
    // validated pass — so we print each perf delta as explicitly UNGATED evidence rather than letting
    // `GRAPHUS_BASELINE_OK` (which gates only the deterministic structural counts below) read as
    // "performance verified" (rmp #743).
    println!(
